...
export const ContextShowSettings = memo(function ContextShowSettings(props) {
    ...
    return shell(_jsxs(_Fragment, { children: [_jsxs("div", { className: styles.fieldRow, children: [_jsx("label", { className: styles.fieldLabel, htmlFor: "context-show-currency", children: t('settings.currency') }), _jsx("select", { id: "context-show-currency", name: "context-show-currency", className: styles.selectInput, value: draft.currency, onChange: (event) => { patch({ currency: event.target.value }); }, children: ['CNY', 'USD', 'CNH', 'EUR', 'GBP', 'JPY'].map((code) => (_jsx("option", { value: code, children: code }, code))) })] }), _jsxs("div", { className: styles.fieldRow, children: [_jsx("label", { className: styles.fieldLabel, htmlFor: "context-show-peak", children: t('settings.peakEnabled') }), _jsx("input", { id: "context-show-peak", name: "context-show-peak", type: "checkbox", className: styles.checkboxInput, checked: showPeak, onChange: (event) => {
                            patch({ peakHours: event.target.checked ? [...DEFAULT_PEAK_RANGES] : [] });
                                        ...
                                        patch({ peakHours: hours });
                                    } })] }, index))) })] })), _jsx("h3", { className: styles.sectionTitle, children: t('settings.defaultPrice') }), _jsx(PriceFieldsEditor, { idPrefix: "context-show-default", value: draft.defaultPrice, showPeak: showPeak, onField: (field, peak, raw) => { patchPrice('defaultPrice', null, field, peak, raw); }, t: t }), _jsx("h3", { className: styles.sectionTitle, children: t('settings.providerPrices') }), Object.entries(draft.prices).map(([key, price]) => (_jsxs("div", { className: styles.entryCard, children: [_jsxs("div", { className: styles.entryHeader, children: [_jsx("input", { className: styles.textInput, id: "context-show-provider-key", name: "context-show-provider-key", value: key, onChange: (event) => { renameKey('prices', key, event.target.value.trim()); } }), _jsx("button", { type: "button", className: styles.removeButton, "aria-label": t('settings.removeProvider', { key }), onClick: () => { removeKey('prices', key); }, children: "\u2715" })] }), _jsx(PriceFieldsEditor, { idPrefix: 'context-show-prices-' + key.replace(/[^a-zA-Z0-9_-]/g, '_'), value: price, showPeak: showPeak, onField: (field, peak, raw) => { patchPrice('prices', key, field, peak, raw); }, t: t })] }, key))), _jsxs("button", { type: "button", className: styles.addButton, onClick: () => { addKey('prices', 'new-provider'); }, children: ["+ ", t('settings.addProvider')] }), _jsx("h3", { className: styles.sectionTitle, children: t('settings.modelPrices') }), Object.entries(draft.modelPrices).map(([key, price]) => (_jsxs("div", { className: styles.entryCard, children: [_jsxs("div", { className: styles.entryHeader, children: [_jsx("input", { className: styles.textInput, id: "context-show-model-key", name: "context-show-model-key", value: key, onChange: (event) => { renameKey('modelPrices', key, event.target.value.trim()); } }), _jsx("button", { type: "button", className: styles.removeButton, "aria-label": t('settings.removeModel', { key }), onClick: () => { removeKey('modelPrices', key); }, children: "\u2715" })] }), _jsx(PriceFieldsEditor, { idPrefix: 'context-show-models-' + key.replace(/[^a-zA-Z0-9_-]/g, '_'), value: price, showPeak: showPeak, onField: (field, peak, raw) => { patchPrice('modelPrices', key, field, peak, raw); }, t: t })] }, key))), _jsxs("button", { type: "button", className: styles.addButton, onClick: () => { addKey('modelPrices', 'provider/model'); }, children: ["+ ", t('settings.addModel')] }), _jsx("p", { className: styles.note, children: t('settings.note') }), _jsxs("div", { className: styles.footer, children: [_jsx("button", { type: "button", className: styles.primaryButton, disabled: !writable || busy, onClick: () => { void onSave(); }, children: t('settings.save') }), _jsx("button", { type: "button", className: styles.secondaryButton, disabled: !writable || busy, onClick: () => { void onReset(); }, children: t('settings.reset') })] })] }));
});
