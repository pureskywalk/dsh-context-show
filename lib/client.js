	...
	factory: (require) => {
		...
		*/
		...
		var ContextShowMeter_module_css_default = {
			...
			"todayRouteRow": "I94Una_todayRouteRow",
		...
		*/
		function todayOf(summary) {
			const value = (summary?.projections?.values)?.["contextUsage"];
			if (typeof value !== "object" || value === null) return void 0;
			const today = value.today;
			if (today === void 0 || typeof today.cost !== "number" || !Number.isFinite(today.cost)) return void 0;
			const routes = [];
			if (Array.isArray(today.routes)) for (const raw of today.routes) {
		...
		*/
		function totalCostOf(summary) {
			const value = (summary?.projections?.values)?.["contextUsage"];
			if (typeof value !== "object" || value === null) return void 0;
			const cost = value.totalCost;
		...
		*/
		const ContextShowMeter = (0, react.memo)(function ContextShowMeter(props) {
			...
			const [remoteSpend, setRemoteSpend] = (0, react.useState)(void 0);
			...
			const priceLinks = (0, react.useMemo)(() => {
				const seen = /* @__PURE__ */ new Map();
				for (const row of providerUsage?.providers ?? []) if (row.priceUrl !== void 0 && !seen.has(row.provider)) seen.set(row.provider, row.priceUrl);
				return [...seen.entries()].map(([provider, url]) => ({
					provider,
					url
				}));
			}, [providerUsage]);
			const sessionsById = useSessions((state) => state.byId);
			const todaySpend = (0, react.useMemo)(() => {
				const currentCwd = sessionsById[sessionId]?.cwd;
				let total = 0;
				let workspace = 0;
				const allRoutes = /* @__PURE__ */ new Map();
				const workspaceRoutes = /* @__PURE__ */ new Map();
				const accumulate = (into, route) => {
					const key = route.provider + "\0" + route.model;
					const existing = into.get(key);
					if (existing === void 0) into.set(key, { ...route });
					else existing.cost += route.cost;
				};
				for (const [id, summary] of Object.entries(sessionsById)) {
					const live = id === sessionId ? providerUsage?.today : void 0;
					const value = live === void 0 ? todayOf(summary) : {
						cost: live.cost,
						routes: live.routes
					};
					if (value === void 0) continue;
					total += value.cost;
					const inWorkspace = currentCwd !== void 0 && summary.cwd === currentCwd;
				...
				const sortRows = (rows) => [...rows.values()].sort((left, right) => right.cost - left.cost || left.model.localeCompare(right.model));
				return {
					total,
					workspace,
					all: sortRows(allRoutes),
					workspaceRows: sortRows(workspaceRoutes)
				};
			}, [
				sessionsById,
				sessionId,
			...
			const workspaceSpend = (0, react.useMemo)(() => {
				const currentCwd = sessionsById[sessionId]?.cwd;
			...
			(0, react.useEffect)(() => {
				...
				const load = async () => {
					try {
						const http = await fetch("/api/dsh-context-show/settings/spend", {
							method: "POST",
							headers: { "content-type": "application/json" },
							body: "{}"
						});
				...
				open,
				sessionId,
				providerUsage?.today?.cost ?? 0
			]);
			const today = (0, react.useMemo)(() => {
				if (remoteSpend === void 0) return {
					date: void 0,
				...
				const currentCwd = sessionsById[sessionId]?.cwd;
				const scope = currentCwd === void 0 ? void 0 : remoteSpend.workspaces.find((candidate) => candidate.cwd === currentCwd);
				return {
					date: remoteSpend.date,
					total: remoteSpend.total.cost,
					totalRoutes: remoteSpend.total.routes,
					workspace: currentCwd === void 0 ? providerUsage?.today?.cost ?? 0 : scope?.cost ?? 0,
					workspaceRoutes: currentCwd === void 0 ? providerUsage?.today?.routes ?? [] : scope?.routes ?? []
				};
			}, [
				remoteSpend,
				sessionsById,
			...
			const workspaceSpendHost = (0, react.useMemo)(() => {
				if (remoteSpend === void 0) return void 0;
				const currentCwd = sessionsById[sessionId]?.cwd;
				if (currentCwd === void 0) return remoteSpend.cumulativeTotal;
			...
			const todayRouteRows = (rows, keyPrefix) => rows.map((route, index) => (0, react_jsx_runtime.jsxs)("div", {
				className: ContextShowMeter_module_css_default.todayRouteRow,
				children: [(0, react_jsx_runtime.jsx)("dt", {
					title: route.provider === "" ? void 0 : route.provider,
					children: route.model === "" ? t("context.unattributed") : route.model
				}), (0, react_jsx_runtime.jsx)("dd", { children: formatMoney(route.cost, currency) })]
			}, keyPrefix + String(index) + route.provider + route.model));
				...
				children: [(0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						(0, react_jsx_runtime.jsxs)("div", {
							...
							}), hasProviderRows && providerUsage !== void 0 && (0, react_jsx_runtime.jsxs)("span", {
								className: ContextShowMeter_module_css_default.costInline,
								children: [(workspaceSpendHost ?? workspaceSpend) !== void 0 && (0, react_jsx_runtime.jsx)("span", {
									className: ContextShowMeter_module_css_default.costLabel,
									children: t("context.workspaceSpendLabel")
								}), formatMoney(workspaceSpendHost ?? workspaceSpend ?? totalCost, providerUsage.currency)]
							})]
						}),
						!detail ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [hasProviderRows && providerUsage !== void 0 && (0, react_jsx_runtime.jsxs)("dl", {
							className: ContextShowMeter_module_css_default.rows,
							children: [(0, react_jsx_runtime.jsxs)("div", {
								className: ContextShowMeter_module_css_default.row,
								children: [(0, react_jsx_runtime.jsx)("dt", { children: t("context.todayWorkspace") }), (0, react_jsx_runtime.jsx)("dd", { children: formatMoney(today.workspace, currency) })]
							}), (0, react_jsx_runtime.jsxs)("div", {
								className: ContextShowMeter_module_css_default.row,
								children: [(0, react_jsx_runtime.jsx)("dt", { children: t("context.todayTotal") }), (0, react_jsx_runtime.jsx)("dd", { children: formatMoney(today.total, currency) })]
							})]
							...
							hasProviderRows && providerUsage !== void 0 && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
								...
								(0, react_jsx_runtime.jsxs)("p", {
									...
									children: [t("context.priceNote"), priceLinks.map((link, index) => (0, react_jsx_runtime.jsxs)("span", { children: [index === 0 ? " · " : " · ", (0, react_jsx_runtime.jsx)("a", {
										...
										children: t("context.officialPriceFor", { provider: link.provider })
									})] }, link.provider))]
								}),
		...
		//#endregion
		//#region lib/client/price-input.js
		/**
		...
		*/
		function parsePriceInput(raw) {
			...
			if (trimmed === "") return 0;
			if (!/^[0-9]*[.]?[0-9]*$/.test(trimmed)) return void 0;
		...
		*/
		function formatPriceInput(current) {
		...
		*/
		...
		const PriceFieldsEditor = (0, react.memo)(function PriceFieldsEditor({ idPrefix, value, showPeak, onField, t }) {
			...
			const cellKey = (field, peak) => field.key + (peak ? ":peak" : ":base");
			const bind = (field, peak, id, current) => ({
				type: "text",
				inputMode: "decimal",
				autoComplete: "off",
				spellCheck: false,
				...
				children: [(0, react_jsx_runtime.jsx)("thead", { children: (0, react_jsx_runtime.jsxs)("tr", { children: [
					...
					return (0, react_jsx_runtime.jsxs)("tr", { children: [
						...
						(0, react_jsx_runtime.jsx)("td", {
							...
							children: (0, react_jsx_runtime.jsx)("input", { ...bind(field, false, offPeakId, value[field.key]) })
						}),
						showPeak && (0, react_jsx_runtime.jsx)("td", {
							className: ContextShowSettings_module_css_default.priceValueCell,
							children: (0, react_jsx_runtime.jsx)("input", { ...bind(field, true, peakId, value.peak?.[field.key] ?? 0) })
						})
					] }, field.key);
				}) })]
			});
		...
		*/
		...
		const zh = {
			...
			"context.officialPriceFor": "{provider} 官方价格 ↗",
			"context.workspaceSpendLabel": "本工作区花费",
			"context.todayWorkspace": "今日 · 本工作区",
			"context.todayTotal": "今日 · 全部会话",
		...
		const en = {
			...
			"context.officialPriceFor": "{provider} pricing ↗",
			"context.workspaceSpendLabel": "Workspace spend",
			"context.todayWorkspace": "Today · this workspace",
			"context.todayTotal": "Today · all sessions",
