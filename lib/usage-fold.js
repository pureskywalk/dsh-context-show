...
export function dayKeyOf(timeMs, timeZone = DEFAULT_TIME_ZONE) {
    return new Intl.DateTimeFormat('en-CA', { timeZone, year: 'numeric', month: '2-digit', day: '2-digit' })
        .format(new Date(timeMs));
}
/** Add one sample's buckets to its day/route cell (returns a fresh `days`). */
function addToDays(days, day, route, tier, buckets) {
    const dayRow = days[day] ?? {};
    const cell = dayRow[route] ?? zeroTierBuckets();
    const nextCell = { peak: cell.peak, offPeak: cell.offPeak };
...
function subFromDays(days, day, route, tier, buckets) {
    const dayRow = days[day];
    const cell = dayRow?.[route];
    if (dayRow === undefined || cell === undefined)
        return days;
    const nextCell = { peak: cell.peak, offPeak: cell.offPeak };
...
function priceOfRouteKey(route, resolve) {
...
function attribute(state, turn, step, usage, isPeak, day) {
    const buckets = bucketsFrom(usage);
    const sample = { turn, step, buckets, tier: isPeak ? 'peak' : 'offPeak', day };
    ...
    if (route === undefined) {
        ...
        if (unchanged && days === state.days
            && bucketsEqual(applied.buckets[applied.last.tier], state.unattributed[applied.last.tier]))
            return state;
