...
/** Fresh empty ledger. */
export const emptyLedger = (): LedgerDoc => ({ version: 1, entries: {} })
...
function parseRoutes(raw: unknown): SpendRouteRow[] {
  if (!Array.isArray(raw)) return []
  const rows: SpendRouteRow[] = []
  for (const candidate of raw) {
    if (typeof candidate !== 'object' || candidate === null) continue
    const row = candidate as { provider?: unknown; model?: unknown; cost?: unknown }
 ...
 */
export function parseLedgerDoc(raw: unknown): LedgerDoc {
  if (typeof raw !== 'object' || raw === null) return emptyLedger()
  const doc = raw as { version?: unknown; entries?: unknown }
  if (doc.version !== 1 || typeof doc.entries !== 'object' || doc.entries === null) return emptyLedger()
  const entries: Record<string, LedgerEntry> = {}
  for (const [id, value] of Object.entries(doc.entries as Record<string, unknown>)) {
    if (typeof value !== 'object' || value === null) continue
    const entry = value as { date?: unknown; cwd?: unknown; cost?: unknown; routes?: unknown; cumulative?: unknown }
 ...
 */
export function observeSession(doc: LedgerDoc, sessionId: string, sample: SpendSample, date: string): LedgerDoc {
  const previous = doc.entries[sessionId]
  if (sample.today === undefined && sample.cumulative === undefined) return doc
 ...
 */
export function ledgerSamples(doc: LedgerDoc): SpendSample[] {
 ...
 */
export async function readLedgerFile(path: string): Promise<LedgerDoc> {
  try {
 ...
 */
export async function writeLedgerFile(path: string, doc: LedgerDoc): Promise<void> {
 ...
 */
export function createLedger(save: (doc: LedgerDoc) => void | Promise<void>, delayMs = 1000): LedgerWriter {
  let doc = emptyLedger()
  ...
  const schedule = (): void => {
    ...
    // Never hold the process open for a ledger write.
