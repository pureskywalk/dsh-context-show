...
export function makeBridgeRoutes(deps: BridgeDeps): WebRoute[] {
  ...
  const mutate = async (request: unknown): Promise<BridgeMutateResult> => {
    ...
    const op = body.op as BridgeSettingsOp
    ...
    try {
      await settings.mutate(namespace, [op] as unknown as SettingsPathOp[], expectedRevision)
    } catch (error) {
