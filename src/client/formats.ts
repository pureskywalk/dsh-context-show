...
export function billedInputTokens(usage: { uncachedInputTokens: number; cacheReadTokens: number; cacheWriteTokens: number }): number {
  return usage.uncachedInputTokens + usage.cacheReadTokens + usage.cacheWriteTokens
}

/** Whole-request cost of one usage bucket set: billed input plus output. */
export function totalTokensOf(usage: { uncachedInputTokens: number; outputTokens: number; cacheReadTokens: number; cacheWriteTokens: number }): number {
 ...
 */
export function cacheHitPercent(usage: { uncachedInputTokens: number; cacheReadTokens: number; cacheWriteTokens: number }): number | null {
...
export function formatMoney(value: number, currency: string): string {
  ...
  if (value < 0.01) return symbol + trimZeros(value.toPrecision(2))
