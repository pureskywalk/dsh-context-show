...
- **花费金额**：按配置单价（每百万 token，默认人民币 CNY）估算的累计花费，**按每次请求的发生时间自动区分高峰 / 非高峰计价**（未启用峰谷时按平价）；详细版按花费降序列出**每个模型**的金额（模型名 + 供应商 + 输入/输出 token 明细），省略版显示 top 2 模型花费与总花费。**在 priceUrls 中显式配置过定价页的 provider**（默认只有 deepseek-official）的官方价格链接集中显示在成本区底部的花费说明行（不再逐模型重复，也不再给未配置的 provider 套用默认链接）。
- **今日花费**：显示「今日 · 本工作区」与「今日 · 全部会话」两个估算值，各自带**按模型的明细**（模型名 + 金额，按花费降序）；**省略版（收起详情）与详细版都会显示**。按配置时区的当天（默认北京时间）统计；当前会话取其投影的实时值，同一工作目录的其它会话取会话列表携带的投影缓存值，因此**从未加载过投影缓存的会话可能暂不计入**（打开一次该会话即会补上）。
